...
@Path("/{imsi}/subsucribe")
public class SubscribeTypeResource {
    private static final Logger LOGGER = LoggerFactory.getLogger(SubscribeTypeResource.class);
    ...
    public Response subscribeFunctionType(@PathParam("imsi") String imsi, @QueryParam("type") String type) {
        ...
        try {
            ...
            if ("holiday".equalsIgnoreCase(type)) {
                DataPage dataPage = userSubscribeTypeService.subscribeAndQueryHoliayTypes(imsi);
                return Response.ok(dataPage).build();
            }

            if ("status".equalsIgnoreCase(type)) {
                DataPage dataPage = userSubscribeTypeService.subscribeAndQueryStatusTypes(imsi);
                return Response.ok(dataPage).build();
            }

            if ("custom".equalsIgnoreCase(type)) {
                DataPage dataPage = userSubscribeTypeService.subscribeAndQueryCustomTypes(imsi);
                return Response.ok(dataPage).build();
            }
        ...
        OperationDescription operationDescription = ObjectFactory.buildErrorOperationDescription(
            Response.Status.NOT_ACCEPTABLE.getStatusCode(), "cancelSubscribeFunctionType", "上传参数异常");
        return Response.ok(operationDescription).build();
    }

    @DELETE
    public Response cancelSubscribeFunctionType(@PathParam("imsi") String imsi, @QueryParam("type") String type) {
        LOGGER.info("Into cancelSubscribeFunctionType imsi = " + imsi);
        LOGGER.info("type = " + type);
        ...
        try {
            ...
            if ("holiday".equalsIgnoreCase(type)) {
                ...
                OperationDescription operationDescription = ObjectFactory.buildOperationDescription(
                    HttpServletResponse.SC_NO_CONTENT, "cancelSubscribeFunctionType", Constants.SUCCESS_FLAG);
                return Response.ok(operationDescription).build();
            }
            ...
            LOGGER.error("cancelSubscribeFunctionType error", e);
            OperationDescription operationDescription = ObjectFactory.buildErrorOperationDescription(
                Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(), "cancelSubscribeFunctionType", "服务器内部错误");
            return Response.ok(operationDescription).build();
        }
        OperationDescription operationDescription = ObjectFactory.buildErrorOperationDescription(
            Response.Status.NOT_ACCEPTABLE.getStatusCode(), "cancelSubscribeFunctionType", "上传参数异常");
