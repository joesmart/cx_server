...
public class ServiceChooseUtil {
    ...
    public String chooseService(String requestUrl, HttpServletRequest req, HttpServletResponse resp, String data) {
        ...
        try {
            ...
            if (requestUrl.equals(Constants.REGISTER_URL)) {
                ...
                String imsi = mapParams.get(Constants.IMSI_STR);
                String type = mapParams.get(Constants.TYPE);
                result = userStatusManagerService.retriveUserStatusUserCXInfoByStatus(imsi, type);
                ...
                String imsi = mapParams.get(Constants.IMSI_STR);
                result = userStatusManagerService.deletCurrentUserStatus(imsi);
            } else {
    ...
    private String deleteUserCXInfo(Map<String, String> mapParams) throws SystemException {
        ...
        Preconditions.checkNotNull(userCXInfoId);
        Preconditions.checkNotNull(imsi);
        
        Long intId = Long.parseLong(userCXInfoId);

        String result = userCXInfoManagerService.deleteUserCXInfo(intId, imsi);
    ...
    /**
     * 根据用户传入的类型和场景去浏览所有彩像的数据
     * @param mapParams
     * @return
     * @throws SystemException 
     */
    private String browserAllData(Map<String, String> mapParams) throws SystemException {
        String result;
        //返回 系统 URL
        Result xmlResult = new Result();

        List<CXInfo> queryResultList = Collections.emptyList();

        queryResultList = cxInfoManagerService.browserAllData(mapParams);
        if (queryResultList != null && queryResultList.size() > 0) {
            xmlResult.setFlag(Constants.SUCCESS_FLAG);
            xmlResult.setCxInfos(queryResultList);
            XMLMarshalUtil marshalUtil = new XMLMarshalUtil(xmlResult);
            result = marshalUtil.writeOut();
        } else {
            result = StringUtil.generateXMLResultString(Constants.DATA_NOTFOUND_FLAG, "数据未找到");
    ...
    /**
     * 从客户端请求中获取所有的参数数据. <p>If necessary, describe how it does and how to use
     * it.</P>
     * @param requestUrl
     * @param req
     * @param data
